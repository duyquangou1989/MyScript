...
#$yesterday  ='2012-08-12';
#$yesterday_format="20120812";
$log_path = '/bkfarm/log';
$destinate_path="/home/ngoalong/www/data/temp";
...
foreach ($conf_servers as $key => $server) 
	{
	    $key = strtoupper($key);
    		if ($server['active']==1 && in_array(strtolower($key),$conf_real_allow_servertrials)) 
		{
			foreach($arr_count as $filename)
			{
				$path="$log_path/$yesterday/{$server['id']}_$key/$yesterday/$filename";
				exec("cat $path | grep UpdateLoginName >> $destinate_path/{$yesterday}_{$key}");
				exec("cat $path | grep CreateRole |grep @guest >> $destinate_path/{$yesterday}_{$key}_total");
					
			}
    		}
	}
 exec("scp -i /root/.ssh/key_scp /home/ngoalong/www/data/temp/{$yesterday}_* root@10.30.27.253:/home/ngoalong/www/data/temp/");
?>
